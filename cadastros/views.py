...
from .models import Nota, Cotacao, Proventos, Ativo
from django.urls import reverse_lazy
from django.contrib.messages.views import SuccessMessageMixin
from bootstrap_datepicker_plus import DatePickerInput
from django.db.models import Count, Sum, Avg
from .method import reader_yahoo
import locale
...
class Dashboard(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        result = {'q':reader_yahoo() }
        return result
    
class ProventosList(LoginRequiredMixin,ListView):
    login_url = reverse_lazy('account_login')
    model = Proventos
    template_name = 'cadastros/listas/proventos.html'
...
class NotaList(LoginRequiredMixin, ListView):
    login_url = reverse_lazy('account_login')
    model = Nota
    template_name = 'cadastros/listas/ordens.html'
...
def notalisttest(request):
    result = Nota.objects.values('ativo').annotate(dcount=Sum('quantidade'))
    context = {
        'q': result,
    }
    return render(request, 'cadastros/listas/proventos.html', context)

def testeFrame(request):
    qs = reader_yahoo()
    data = pd.DataFrame(qs)
    context = {
        'df' : data.to_html()
    }
    return render(request, 'cadastros/listas/dashboard.html', context)
...
def dash(request):
    return render(request,'cadastros/listas/dashboard.html')
